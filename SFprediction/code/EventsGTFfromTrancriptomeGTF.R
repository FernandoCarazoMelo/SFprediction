...
EventGTFfromTrancriptomeGTF <- function(inputFile = NULL,Transcriptome = NULL, Pathtxt=NULL, PathGTF=NULL){
  ...
  ############################ 
  # Metodo de SG_Seq original 
  ############################ 
  # genes <- unique(unlist(geneID(SplicingGraphFeatures)))
  # 
  # GeneIDs <- as.data.frame(SplicingGraphFeatures)
  ...
  # 
  # 
  # # Result <- vector("list", length = length(genes))
  # Result <- vector("list", length = 50)
  # # seq_along(genes)
  # for (jj in 1:50) {
  #   
  #   cat(jj,' of 50 \n')
  ...
  #   Gene <- genes[jj]
  #   
  #   SG_Gene <- SplicingGraphFeatures[unlist(geneID(SplicingGraphFeatures)) == Gene]
  #   
  ...
  #           # event
  #           GeneName <- geneName(SG_Gene)[[1]]
  #           GenI <- geneID(SG_Gene)[[1]]
  #           Events <- AnnotateEvents_KLL(Events, GeneName,GenI)
  #           
